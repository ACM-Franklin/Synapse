...
public class GuildHistoricalScanner {
     ...
     * @return a future that completes with the updated channel/thread checkpoints
     */
    public CompletableFuture<Map<Long, Long>> scanGuild(Guild guild, Map<Long, Long> lastSeenByChannel) {
        return scanGuild(guild, lastSeenByChannel, (channelExtId, lastMessageExtId) -> {
        });
    }

    CompletableFuture<Map<Long, Long>> scanGuild(Guild guild, Map<Long, Long> lastSeenByChannel,
                                                 BiConsumer<Long, Long> checkpointRecorder) {
        return CompletableFuture.supplyAsync(() -> {
            Map<Long, Long> updatedCheckpoints = new HashMap<>(lastSeenByChannel);
            ...
            for (MessageChannel channel : channels) {
                ...
                scanChannelSync(channel, startAfter, updatedCheckpoints, checkpointRecorder);
            }
            ...
            for (ForumChannel forum : guild.getForumChannels()) {
                ...
                scanForumSync(forum, updatedCheckpoints, checkpointRecorder);
            }
            ...
            for (ThreadChannel thread : guild.getThreadChannelCache()) {
                ...
                scanThreadSync(thread, parentChannelInternalId, startAfter,
                    updatedCheckpoints, checkpointRecorder);
            }
            ...
            for (IThreadContainer container : threadContainers) {
                scanArchivedThreads(container, updatedCheckpoints, checkpointRecorder);
            }

            return Map.copyOf(updatedCheckpoints);
        });
    }
    ...
    private void scanChannelSync(MessageChannel channel, long startAfterId,
                     Map<Long, Long> updatedCheckpoints,
                     BiConsumer<Long, Long> checkpointRecorder) {
        long channelInternalId = channelService.upsertChannel(channel);
        long count = scanMessagesSync(channel, channelInternalId, null,
            startAfterId, updatedCheckpoints, checkpointRecorder);
    ...
    private void scanThreadSync(ThreadChannel thread, long parentChannelInternalId, long startAfterId,
        ...
        long count = scanMessagesSync(thread, parentChannelInternalId, threadInternalId,
            startAfterId, updatedCheckpoints, checkpointRecorder);
     ...
     */
    private void scanForumSync(ForumChannel forum, Map<Long, Long> updatedCheckpoints,
                               BiConsumer<Long, Long> checkpointRecorder) {
        ...
        for (ThreadChannel post : forum.getThreadChannels()) {
            long startAfter = updatedCheckpoints.getOrDefault(post.getIdLong(), 0L);
            log.info("  Scanning active forum post: {} (ID: {})", post.getName(), post.getIdLong());
            scanThreadSync(post, forumInternalId, startAfter, updatedCheckpoints, checkpointRecorder);
        }
     ...
     */
    private void scanArchivedThreads(IThreadContainer container, Map<Long, Long> updatedCheckpoints,
                                     BiConsumer<Long, Long> checkpointRecorder) {
        ...
        try {
            ...
            for (ThreadChannel thread : archived) {
                long startAfter = updatedCheckpoints.getOrDefault(thread.getIdLong(), 0L);
    ...
    private long scanMessagesSync(MessageChannel channel, long channelInternalId,
        ...
        while (true) {
            ...
            updatedCheckpoints.put(channel.getIdLong(), currentAfterId);
            checkpointRecorder.accept(channel.getIdLong(), currentAfterId);
